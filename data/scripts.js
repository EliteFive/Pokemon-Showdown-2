exports.BattleScripts = {
	...
	randomDoublesTeam: function(side) {
		...
		for (var i=0; i<keys.length && pokemonLeft < 6; i++) {
			...
			if (template.id === 'hitmontop') {
				template.viableMoves = {closecombat:1, suckerpunch:1, wideguard:1, fakeout:1, helpinghand:1, machpunch:1};
				var items = ['Sitrus Berry', 'Fighting Gem'];
				items = items.randomize();
				set.item = items[0];
				set.ability = 'Intimidate';
				set.level = 74;
			} else if (template.id === 'abomasnow') {
				template.viableMoves = {blizzard:1, gigadrain:1, iceshard:1, protect:1, grassknot:1};
				set.item = 'Focus Sash';
