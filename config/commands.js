...
var commands = exports.commands = {
	...
	groups: function(target, room, user) {
		if (!this.canBroadcast()) return;
		this.sendReplyBox('+ <b>Factory Worker</b> - The hard workers who are key in doing excessive labor that others are too lazy to do theirselves. They can also use ! commands like !groups, and talk during moderated chat<br />' +
			'% <b>Factory Technician</b> - The factory\'s personal repair man. He can do the above, and can also mute users and check for alts<br />' +
			'@ <b>Factory Supervisor</b> - Those people that allow the directors to take vacation in Jamaica 50% of the year. They can do the above and they can lock users<br />' +
			'& <b>Assistant Director</b> - They help give the main director time to sleep in on weekends. They can do the above and they can also promote supervisors and ban users<br />'+
			'~ <b>Factory Director</b> - The factory\'s big boss, also known as Nollan. He can do anything, like change what this message says');
	},
	//Here is my custom tier command, enjoy! ^_^
	tier: 'tiers',
	metagame: 'tiers',
	metagames: 'tiers',
	tiers: function(target, room, user) {
		...
		if (target === 'lc' || target === 'littlecup') {
            		matched = true;
            		this.sendReplyBox('- <a href="http://www.smogon.com/bw/metagames/lc" target="_blank">LC Metagame Analyses</a><br />' +
            			'- <a href="http://www.smogon.com/forums/forumdisplay.php?f=161" target="_blank">LC\'s Smogon Forums Thread</a><br />');
		}
		if (target === 'doublesvgc' || target === 'vgc2013' || target === 'vgcdoubles') {
            		matched = true;
            		this.sendReplyBox('- <a href="http://www.smogon.com/bw/metagames/vgc2012" target="_blank">VGC 2012 Metagame Analyses</a><br />' +
            			'- <a href="http://www.smogon.com/forums/forumdisplay.php?f=127" target="_blank">VGC\'s Smogon Forums Thread</a><br />');
		}
		...
		if (target === 'vgcsingles' || target === '3vs3singles') {
            		matched = true;
            		this.sendReplyBox('In VGC Singles, you choose 3 out of 6 of your pokemon upon the start of the battle,and this leads to a faster paced metagame that functions somewhat like VGC Doubles. In addition, the VGC Doubles banlist is followed, so things like Blaziken are allowed.');
		}
		if (target === 'oulenient' || target === 'oulimitedbanlist') {
            		matched = true;
            		this.sendReplyBox('OU Lenient is basically OU with a minimal banlist. It is based on Joim\'s OU Lenient, but has a slightly different banlist.<br />' +
            			'As of this moment, the following pokemon are banned: Kyogre, All Arceus Formes, Mewtwo, Palkia, Rayquaza, Dialga, Kyurem-White, Reshiram, Zekrom, Deoxys-Attack, Giratina, Giratina-Origin, Groudon, Ho-oh, and Lugia.');
		}
		if (target === 'oumonotype') {
            		matched = true;
            		this.sendReplyBox('Well I am fairly shocked you do not know what OU Monotype is, but OU Monotype is simply the standard OU metagame, except that every pokemon on your team must share one type.');
		}
		if (target === 'uberdoubles' || target === 'doubleubers') {
            		matched = true;
            		this.sendReplyBox('Uber Doubles is exactly what the name says: Ubers in Doubles format.');
		}
		if (target === 'nudoubles') {
            		matched = true;
            		this.sendReplyBox('- <a href="http://www.smogon.com/smog/issue26/nu_doubles" target="_blank">NU Doubles Overview</a><br />');
		}
		if (target === 'npmou' || target === 'npmubers' || target === 'npm') {
            		matched = true;
            		this.sendReplyBox('NPM is a pokemon mod that aims to value flavor more than most other mods. In addition, it explores new typings, and how new abilities and moves affect the OU metagame.<br />' +
            			'- <a href="http://pastebin.com/XjN3MQeU" target="_blank">Current NPM changelist</a><br />' +
            			'- <a href="http://piratepad.net/NPMOU" target="_blank">Have any suggestions or comments? Leave them here</a>');
		}
		if (target === 'dscap') {
            		matched = true;
            		this.sendReplyBox('- <a href="http://piratepad.net/DSCAPCreation" target="_blank">DSCAP Main Page</a><br />');
		}
		if (target === 'hackmons') {
            		matched = true;
            		this.sendReplyBox('- <a href="http://www.smogon.com/forums/showthread.php?t=3475624" target="_blank">The Hackmons Thread</a><br />');
		}
		if (target === 'balancedhackmons' || target === 'bh') {
            		matched = true;
            		this.sendReplyBox('- <a href="http://www.smogon.com/forums/showthread.php?t=3463764" target="_blank">The Balanced Hackmons Thread</a><br />');
		}
		if (target === 'bhdoubles' || target === 'balancedhackmondoubles') {
            		matched = true;
            		this.sendReplyBox('BH Doubles is Balanced Hackmons in Doubles format. As of now, the metagame is unknown and as such there are not very many bans at this time, though Dark Void and Sky Drop will probably end up being banned.');
		}
		if (target === 'glitchmons') {
            		matched = true;
            		this.sendReplyBox('- <a href="http://www.smogon.com/forums/showthread.php?t=3467120" target="_blank">The Glitchmons Thread</a><br />');
		}
		if (target === 'gennext' || target === 'gennextou') {
            		matched = true;
            		this.sendReplyBox('- <a href="https://github.com/Zarel/Pokemon-Showdown/tree/master/mods/gennext" target="_blank">GenNEXT changelist</a><br />');
		}
		if (target === 'tiershift' || target === 'ts') {
            		matched = true;
            		this.sendReplyBox('- <a href="http://www.smogon.com/forums/showthread.php?t=3479358" target="_blank">Tier Shift Other Metagame Thread</a><br />');
		}
		if (target === 'ousixmoves' || target === 'sixmoveou') {
            		matched = true;
            		this.sendReplyBox('- <a href="http://www.smogon.com/forums/showthread.php?t=3470224" target="_blank">OU Six Moves Other Metagame Thread</a><br />');
		}
		if (target === 'reversestat' || target === 'negativestat' || target === 'negativestats' || target === 'reversestats') {
            		matched = true;
            		this.sendReplyBox('- <a href="http://www.smogon.com/forums/showthread.php?t=3483195" target="_blank">Reverse Stats Other Metagame Thread</a><br />');
		}
		if (target === 'statexchange') {
            		matched = true;
            		this.sendReplyBox('- <a href="http://www.smogon.com/forums/showthread.php?t=3482791" target="_blank">Stat Exchange Other Metagame Thread</a><br />');
		}
		if (target === 'statswap') {
            		matched = true;
            		this.sendReplyBox('Stat Swap is similar to Stat Exchange. In Stat Swap, every pokemon has its offenses, defenses, and HP/Speed swapped, creating a very unique metagame.');
		}
		/*if (target === 'v2' || target === 'v2project' || target === 'nuv2' || target === 'ruv2' || target === 'uuv2' || target === 'ouv2') {
            		matched = true;
            		this.sendReplyBox('- <a href="https://docs.google.com/document/d/1lhU_jGa8_0TiEcN4w6K3k2xvIN5eeflLvarUfwW4QrI/edit?pli=1" target="_blank">Current v2 Changelist</a><br />');
		}*/
		if (target === 'skybattle' || target === 'skybattleou' || target === 'doublesskybattle') {
			matched = true;
			this.sendReplyBox('A Sky Battle is simply a battle in the sky, which means only Flying-types and pokemon with Levitate can participate. There are both Singles and Doubles formats for this. Note that Soul Dew is banned, and Sky Drop is banned in Doubles.');
		}
		if (!matched) {
	...
	//END OF MY CUSTOM COMMANDS
	opensource: function(target, room, user) {
		if (!this.canBroadcast()) return;
		this.sendReplyBox('Pokemon Showdown is open source:<br />- Language: JavaScript<br />- <a href="https://github.com/Zarel/Pokemon-Showdown/commits/master" target="_blank">What\'s new?</a><br />- <a href="https://github.com/Nollan101/Pokemon-Showdown" target="_blank">Nollan\'s Factory code</a><br />- <a href="https://github.com/Zarel/Pokemon-Showdown-Client" target="_blank">Client source code</a>');
	},

	avatars: function(target, room, user) {
		if (!this.canBroadcast()) return;
		this.sendReplyBox('Your avatar can be changed using the Options menu (it looks like a gear) in the upper right of Pokemon Showdown. Custom avatars cannot be accessed, and only Administrators (~) can give those to users.');
	},

	introduction: 'intro',
	intro: function(target, room, user) {
		if (!this.canBroadcast()) return;
		this.sendReplyBox('Welcome to Nollan\'s Pokemon Showdown server!<br />' +
			'There are a few key differences or suggestions on my server as follows:<br />' +
			'- The /tier command allows you to check what specific tiers are and how they work. Proper syntax is /tier "nameoftier".<br />' +
			'- The time of day in which the most people are around on the server is about 7:00 Central, so that is the best time to be on.<br />' +
			'- Groups have different command capabilities, for more details, use /groups. In addition, while still a work in progress, all users with a "%" status or higher gain access to an additional chatroom.<br />' +
			'- As a last note, Nollan is often afk or on another tab, so if you wish to speak with him, you may have to wait for awhile.</a>');
	},
